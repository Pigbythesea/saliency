"""SALICON manifest-based dataset loader."""
...
class SALICONDataset(BaseVisionDataset):
    """SALICON loader backed by a portable manifest CSV."""
    ...
    @classmethod
    def from_config(cls, config: dict[str, Any]) -> "SALICONDataset":
        return cls(
            root=config.get("root", "data/salicon"),
    ...
    def _load_rows(self) -> list[_SALICONRow]:
        if not self.manifest_path.is_file():
            raise FileNotFoundError(f"SALICON manifest not found: {self.manifest_path}")

        rows: list[_SALICONRow] = []
        with self.manifest_path.open("r", encoding="utf-8", newline="") as handle:
            ...
            if missing:
                raise ValueError(
                    f"SALICON manifest missing required columns: {sorted(missing)}"
                )
    ...
    def _validate_files_exist(self) -> None:
        for row in self.rows:
            if not row.image_path.is_file():
                raise FileNotFoundError(f"SALICON image not found: {row.image_path}")
...
register_dataset("salicon", SALICONDataset)
register_dataset("SALICON", SALICONDataset)
