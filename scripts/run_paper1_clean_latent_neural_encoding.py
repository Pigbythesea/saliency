"""Validate or run the Paper 1 clean latent neural encoding lane."""
...
OUTPUT_KEYS = ["encoding_scores", "audit", "failure_log"]
...
def run(config_path: str, *, dry_run: bool) -> int:
    ...
    model_rows = eligible_model_rows("latent_neural_encoding")
    layer_map = model_layer_map(config)
    planned_rows = build_planned_rows(model_rows, subject_roi_rows, layer_map)
    audit_rows = [
        audit_row(
            lane=LANE,
            check_id="publication_paths",
            status="pass",
            artifact="neural_outputs",
            path=pipe_join(str(path.relative_to(PROJECT_ROOT)) for path in outputs.values()),
            detail="configured neural outputs are under outputs/paper1_publication_v0",
        ),
        audit_row(
            lane=LANE,
        ...
        audit_row(
            lane=LANE,
            check_id="planned_encoding_rows",
            status="pass",
            artifact="neural_plan",
            path=dry_run_report_path(LANE).relative_to(PROJECT_ROOT),
            detail=f"planned_model_subject_roi_layer_rows={len(planned_rows)}",
        ),
    ]
    ...
    if dry_run:
        report = write_dry_run_report(
            ...
            extra={
                ...
                "subject_roi_pairs": len(subject_roi_rows),
            },
        )
        print(f"dry_run_passed={report.relative_to(PROJECT_ROOT)}")
        return 0
    cell_files = clean_cell_score_files(outputs["encoding_scores"].parent / "cells")
    rows = materialize_encoding_scores(
        cell_files,
        model_rows=model_rows,
        subject_roi_rows=subject_roi_rows,
    )
    write_csv(outputs["encoding_scores"], rows)
    audit_rows.append(
        audit_row(
            lane=LANE,
            check_id="encoding_scores_written",
            status="pass",
            artifact="encoding_scores",
            path=outputs["encoding_scores"].relative_to(PROJECT_ROOT),
            detail=f"source_cell_files={len(cell_files)}; rows={len(rows)}",
        )
    )
    write_lane_audit(outputs["audit"], audit_rows)
    print(outputs["encoding_scores"].relative_to(PROJECT_ROOT))
    return 0
...
def clean_cell_score_files(root: Any) -> list[Any]:
    ...
    files = []
    for path in sorted(root.rglob("encoding_scores.csv")):
        reject_forbidden_path(path, "neural_encoding.cell_score")
        files.append(path)
    if not files:
        raise CleanRerunValidationError(
            "No clean neural encoding cell scores found under "
            f"{root.relative_to(PROJECT_ROOT)}. Generate clean neural cell jobs first."
        )
    return files


def materialize_encoding_scores(
    cell_files: list[Any],
    *,
    ...
    if not output_rows:
        raise CleanRerunValidationError("Clean neural encoding cell files contained no rows")
    return output_rows


def available_subject_roi_rows(config: dict[str, Any]) -> list[dict[str, str]]:
    roi_config = dict(config.get("roi_stream", {}))
    ...
    if not rows:
        raise CleanRerunValidationError("No available subject/ROI rows for neural encoding")
    return rows
...
def model_layer_map(config: dict[str, Any]) -> dict[str, list[str]]:
    ...
    if not layers:
        raise CleanRerunValidationError("Latent neural config has no model layers")
    return layers
                ...
                rows.append(
                    {
                        **model_identity(model),
                        ...
                        "layer": layer,
                        "evidence_type": "latent_feature_neural_encoding",
